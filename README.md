...
**1.** Redpanda is supported as a source in Materialize through the Kafka source, so the first step is to create a [Kafka+JSON source](https://materialize.com/docs/sql/create-source/json-kafka/) and point it to the `redpanda` broker:

```sql
CREATE SOURCE rp_twitch
